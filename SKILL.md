...
description: >
  ...
  17条红线自审、7种论文类型自适应、WYSIATI认知校准、证据先行架构、术语账本、
  洞察依赖图、19节固定教学输出模式。精读：任意论文任意页面，以"零基础→完全理解"
...
## 0.4 Red Lines Catalog — 红线目录（17条可测试禁令）
...
**转换规则**：
1. **公式代码块**（``` ``` 围栏内容为公式的）→ 去掉围栏，线性化为**单行** `$...$`；公式编号保留在 `$` 内（如 `\quad (9)`）。
2. **希腊字母**：α→`$\alpha$` β→`$\beta$` γ→`$\gamma$` θ→`$\theta$` λ→`$\lambda$` μ→`$\mu$` π→`$\pi$` σ→`$\sigma$` τ→`$\tau$` φ→`$\phi$` χ→`$\chi$` ψ→`$\psi$` ω→`$\omega$`；大写 Γ→`$\Gamma$` Δ→`$\Delta$` Σ→`$\Sigma$`（求和语境用 `$\sum$`）等。
3. **上标/下标 Unicode**：x²→`$x^2$` 10⁻²⁸→`$10^{-28}$` g₀→`$g_0$` C_k→`$C_k$` I_k³→`$I_k^3$`；组合形式如 `h^AG_{m,u}`→`$h^{AG}_{m,u}$`。
4. **数学运算符**：·→`$\cdot$` ×→`$\times$` ∝→`$\propto$` ≤→`$\leq$` ≥→`$\geq$` ≈→`$\approx$` ≠→`$\neq$` ∈→`$\in$` ∞→`$\infty$`。
5. **符号表**：Markdown 表格"符号"列的每个数学 token 包 `$...$`。
6. **已有 `$$...$$` display math** → 线性化为单行 `$...$`（去掉 `$$` 围栏，`\frac`/`\hat`/`\overline`/`\odot` 等 LaTeX 命令原样保留）。
7. **除号 `/` 优先保留**（可读性好）；仅在原式已是分式结构时用 `\frac{}{}`。

**不强制转换（豁免区）**：ASCII 教学方框图/流程图（`│▼└─→` 组成）、算法伪代码/方法草图代码块、`[SIMPLIFIED]`/`[VISUALLY CONFIRMED]` 等方括号标记内的数学、货币 `$`（如 `US$`）、普通数字单位（`3倍` `27分钟` `0.5秒` `40%`）。
**测试**：每行 `$` 数为偶数（无孤立 `$`）？正文（豁免区外）无裸露希腊字母/上下标 Unicode（²³⁰₁）？数学代码块围栏是否已移除？

### 红线自审协议（🔴 内部门禁 — 不写入交付文档）

红线是**内部质量门**，用于防止错误产出，**绝不写进交付给用户的 Markdown 文档**。用户明确要求（2026-08-14）：红线只作为内部自检，不出现在精读文档中。

每次输出交付前：
1. 依次检查 R1 到 R17（内部执行）
2. 发现违规 → 修复后再交付；无法修复的以内部备注记录，不外露
3. 交付文档中**不再出现**"红线自审摘要/红线检查/17条红线"等字样
...
**🔴 内联化强制（v2.4，R17）**：本文档中的所有公式和数学符号一律用 `$...$` 内联 LaTeX 书写——公式代码块压成单行 `$...$`，希腊字母/上下标/运算符全部转 `\alpha` `^2` `\times` 等形式，符号表"符号"列逐 token 包裹。禁止纯文本/Unicode 数学裸奔。豁免：ASCII 教学图、算法伪代码、`[SIMPLIFIED]` 等标记内部。详见红线 R17。
...
### 🔴 强制前置步骤：PDF 图表渲染（MANDATORY — v2.3 新增）

> **"你不能描述你没看到的东西。猜 = 撒谎。"**

**在尝试描述任何 PDF 中的图表之前，必须先执行渲染步骤。违规 = R16 红线违反。**
...
Step R1: 定位图表所在页码
  → 从 Pass 1 的结构地图中获取 Fig.N 所在的 PDF 页码

Step R2: 使用 pymupdf (fitz) 渲染该页为 PNG
  → python -c "import fitz; doc=fitz.open('PATH'); p=doc[PAGE_NUM-1]; pix=p.get_pixmap(dpi=200); pix.save('OUT.png')"
...
渲染图片后，逐项确认以下内容，每一项必须基于实际看到的内容回答：

| # | 检查项 | 问题 | 错误示例 |
|:---:|------|------|------|
| 1 | **图表类型确认** | 这是什么类型的图？（折线/散点/柱状/CDF/热力图/示意图） | 把折线图说成柱状图 ❌ |
| 2 | **坐标轴标签** | X轴和Y轴分别标注了什么？单位是什么？ | 猜坐标轴含义而不看实际标签 ❌ |
| 3 | **数据系列数量** | 图中有几条线/几组柱子/几个数据系列？ | 说"三条线"但实际只有两条 ❌ |
| 4 | **图例对应** | 每条线/每组柱子对应什么方法/条件？ | 把A方法的线和B方法的线搞反 ❌ |
| 5 | **数值范围** | Y轴的数值范围是多少？关键数据点大约在什么数值？ | 说"1到100"但实际Y轴是0到1 ❌ |
...
```
🔴 R5-extended: 绝不在没有看到渲染图片的情况下声称图表的:
  - 图表类型（"这是一个柱状图"）
  - 坐标轴含义（"X轴是时间"）
  - 数据趋势（"MATC的线在Q-learning上面"）
  - 具体数值（"覆盖得分约为0.95"）

✅ 如果渲染失败: 只能引用论文正文中对图表的文字描述，并标注 [TEXT-ONLY]
✅ 如果渲染成功: 每条观察必须标注 [VISUALLY CONFIRMED]
```

#### 6.0.4 颜色与可访问性警告（继承自 SciPilot Figure Skill）

基于 SciPilot 的 Okabe-Ito 色盲安全调色板，检查论文图表：

| SciPilot 规则 | 在论文读图中的含义 |
|-------------|-----------------|
| Okabe-Ito 默认配色 | 橙色 `#E69F00`、天蓝 `#56B4E9`、蓝绿 `#009E73`、黄 `#F0E442`、蓝 `#0072B2`、朱红 `#D55E00`、粉紫 `#CC79A7` |
| 禁止纯颜色区分 | 如果论文只用颜色区分线条（无线型区分），标注 [COLOR-ONLY] → 色盲读者可能完全无法区分 |
| 禁止 rainbow/jet | 如果热力图用了jet色图，标注 [JET-COLORMAP] → 黄色和青色之间的"边界"是视觉假象 |

---

### 6.1 图表解读（视觉确认后执行）

**仅在完成 6.0 渲染和视觉检查后，执行以下解读流程：**
...
Step 3: 趋势描述（用 ASCII 示意曲线走向，基于实际看到的趋势绘制）
  标注关键拐点和数值范围 [VISUALLY CONFIRMED]
...
**v2.3 新增**：对每张图评估证据强度（强/中/弱），与声明-证据矩阵保持一致。
**v2.3 新增**：如使用了 [TEXT-ONLY] 模式（渲染失败），必须在读图卡开头显式标注并说明原因。
...
18. 论文优点发现（五维亮点）：写作语言 / 建模设计 / 思考路径 / Idea生成过程 / 问题发现——"这篇论文好在哪、值得我学什么"
19. 💡 新 Idea 发现：本论文可拓展的研究方向（过4道品质关卡）
```
> 教学图索引（Python/mermaid/ASCII 自创图清单）已移出编号结构，如需可作为文档末尾附录。
...
## 第18节详解：论文优点发现（五维亮点）🔴 v2.5 新增

> 用户需求（2026-08-14）：每次喂论文后，除了教学精读，还要**总结这篇论文的优点**（写作语言 / 建模 / 思路 / 想到idea的过程 / 问题发现），并且我自己也要**学习**这些优点。

第18节「论文优点发现」不是恭维，是**可学习的结构化亮点萃取**。五个维度，每个维度必须有具体证据（锚定论文 §X / 原文句子），禁止空话。

### 五维拆解

| 维度 | 要回答的问题 | 证据示例 |
|------|------------|---------|
| **1. 写作语言** | 作者怎么把复杂问题讲清楚？有什么值得借鉴的表达技巧？ | "用类比开场" "问题公式化前先给直觉" "Motivation用故事钩子" "实验图表设计一眼看懂" |
| **2. 建模** | 问题建模的巧妙之处？假设的取舍？数学工具的选用？ | "把NP难问题分解为凸子问题" "用连续松弛降低复杂度" "奖励函数设计成可分解" "信道建模用实测数据" |
| **3. 思考路径** | 作者的解题思路主线？先解决什么后解决什么？为什么这样推进？ | "先解耦再联合" "先单机后多机" "从仿真到实测的渐进" "用消融实验定位关键组件" |
| **4. Idea生成过程** | 作者是怎么从问题想到这个idea的？哪些坑被巧妙绕开？ | "从Future Work反向推导" "看到多智能体缺陷→引入集中式Critic" "用真实数据驱动设计" |
| **5. 问题发现** | 作者怎么发现这个研究问题？为什么这是个好问题？ | "观察到无人机轨迹优化与通信质量脱节" "把部署痛点转成可解数学问题" "定位前人只控单机的空白" |

### 输出要求

- 每个维度 2~4 条，每条一句话 + 锚定证据 + 一句"值得我学什么"。
- 放在第 18 节，作为「这篇论文好在哪」的总结，与第 19 节 Idea 衔接（优点 → 启发新 Idea）。
- 禁止泛泛而谈（"写作清晰""建模合理"这种空话不算），必须具体到可复制的方法。
...
## 自我学习机制（我如何从每篇论文成长）🔴 v2.5 新增

> 用户需求（2026-08-14）："你自己也进行学习"。选定的保存方式 = **只更新 skill 记忆**（不单独建每篇笔记）。
